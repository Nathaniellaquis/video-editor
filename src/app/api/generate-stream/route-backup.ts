      ...
      try {
        ...
        if (format === 'short' || format === 'both') {
          ...
          await new Promise<void>((resolve, reject) => {
            ...
            let maskOffset = 2; // Start counting from input 2
            if (bgPath) {
            ...
            filterArray.push(`color=c=${bgColor}:s=1080x1920[bg]`);
            ...
            const screenW = 1080;
            const screenH = 669;
            const screenPadW = Math.ceil(screenW / 2) * 2;
            const screenPadH = Math.ceil(screenH / 2) * 2;
            
            // Scale to target size
            filterArray.push('[0:v]scale=w=1080:h=669:force_original_aspect_ratio=increase,crop=1080:669[screen_scaled]');
            // Pad to even dimensions if needed
            filterArray.push(`[screen_scaled]pad=w=${screenPadW}:h=${screenPadH}:x=0:y=0:color=black,format=yuva420p[screen_raw]`);
            
            // Process mask - pad to match video exactly
            filterArray.push(`[${maskOffset}:v]pad=w=${screenPadW}:h=${screenPadH}:x=0:y=0:color=black@0,format=rgba[screenmask_padded]`);
            filterArray.push('[screenmask_padded]alphaextract[screenmask]');
            filterArray.push('[screen_raw][screenmask]alphamerge,format=yuva420p[screen]');
            
            // Process face recording with top rounded corners - dynamic padding for any dimension
            // Calculate even dimensions for consistency
            const faceW = 1080;
            const faceH = 939;
            const facePadW = Math.ceil(faceW / 2) * 2;
            const facePadH = Math.ceil(faceH / 2) * 2;
            
            // Scale to target size
            filterArray.push('[1:v]scale=w=1080:h=939:force_original_aspect_ratio=increase,crop=1080:939[face_scaled]');
            // Pad to even dimensions if needed
            filterArray.push(`[face_scaled]pad=w=${facePadW}:h=${facePadH}:x=0:y=0:color=black,format=yuva420p[face_raw]`);
            
            // Process mask - pad to match video exactly
            filterArray.push(`[${maskOffset + 1}:v]pad=w=${facePadW}:h=${facePadH}:x=0:y=0:color=black@0,format=rgba[facemask_padded]`);
            filterArray.push('[facemask_padded]alphaextract[facemask]');
            filterArray.push('[face_raw][facemask]alphamerge,format=yuva420p[face]');
            ...
            command
              ...
              .inputOptions([
                '-stream_loop', '0'  // Don't loop input streams
              ])
              .save(shortFormPath);
            ...
            command
              ...
              .on('stderr', (stderrLine) => {
                console.log('Short form FFmpeg stderr:', stderrLine);
              })
        ...
        if (format === 'long' || format === 'both') {
          ...
          await new Promise<void>((resolve, reject) => {
            ...
            filterArray.push(`color=c=${bgColor}:s=1920x1080[bg]`);
            ...
            if (bgPath) {
              ...
              // Scale face for PiP
              filterArray.push('[1:v]scale=365:206:force_original_aspect_ratio=decrease,pad=365:206:(ow-iw)/2:(oh-ih)/2[face]');
